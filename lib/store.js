...
var Store = (_class = function () {
	function Store(props) {
		...
		if (isNode) {
			...
			this.emitter = {};
			var emitPromise = new Promise(function (resolve) {
				_this2.stream = _kefir2.default.stream(function (emitter) {
			...
			this.emitter.emit = function (params) {
				emitPromise.then(function () {
					_this2.emitter.emit(params);
				});
			};
		}
	...
	(0, _createClass3.default)(Store, [{
		...
		value: function getDataIn(store) {
			...
			return function (path, info) {
				...
				var value = localData;
				if (path) {
					value = (0, _getPathParse2.default)(path, value, info).value;
     ...
     */
       ...
       */
							...
							if (v._getData && typeof v._getData === 'function') {
								_result = v._getData(undefined, v.getDataIn(), info);
								v.getCounter();
							} else {
								v.beforeGet();
								_result = v.getData(undefined, v.getDataIn(), info);
								v.getCounter();
								v.afterGet(_result);
							}
							return _result;
		...
		key: 'getData',
		value: function getData(path, info) {
		...
		key: 'getStore',
		value: function getStore(path, info) {
			if (!path) {
     ...
     */
		...
		key: 'setData',
		value: function setData(value, path, options, info, res) {
			var _this6 = this;
		...
		value: function setStore(value, path, options, info) {
			...
			return new Promise(function (res) {
				if (isNode) {
					_this7.emitter.emit('event', [value, path, options, info, res]);
				} else {
					_this7.emitter.emit([value, path, options, info, res]);
				}
			});
		}
