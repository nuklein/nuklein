...
function stringPathParse(path, data, insPath, onDataNotFoundAll, onDataNotFound) {
	var value = data;
	var keys = path.split('.');
	var exit = false;
    ...
    */
				if (localValue._getData && typeof localValue._getData === 'function') {
					value = localValue._getData(newPath, localValue.getDataIn(), true, onDataNotFoundAll, onDataNotFound);
					localValue.getCounter();
				} else {
					localValue.beforeGet();
					value = localValue.getData(newPath, localValue.getDataIn(), true, onDataNotFoundAll, onDataNotFound);
					localValue.getCounter();
...
function arrayPathParse(path, data, onDataNotFoundAll, onDataNotFound, originalPath, keys, nfPath) {
	var notFoundPath = nfPath;
	return {
		value: (0, _immutable.List)(path.map(function (v, k) {
			var res = getPathParse(v, data, onDataNotFoundAll, onDataNotFound, originalPath || path, keys ? keys.concat(k) : [k], notFoundPath);
...
/* eslint-disable no-use-before-define */
function objectPathParse(path, data, onDataNotFoundAll, onDataNotFound, originalPath, keys, nfPath) {
	var localData = data;
	var notFoundPath = nfPath;
	var insPath = (0, _extends3.default)({}, path, { _path: undefined });
	if (path._path) {
		var strRes = stringPathParse(path._path, data, insPath, onDataNotFoundAll, onDataNotFound);
	...
	Object.keys(path).forEach(function (key) {
		if (key !== '_path') {
			var res = getPathParse(path[key], localData, onDataNotFoundAll, onDataNotFound, originalPath || path, keys ? keys.concat(key) : [key], notFoundPath);
			var v = res.value;
			notFoundPath = res.notFoundPath || nfPath;
...
function getPathParse(path, data, onDataNotFoundAll, onDataNotFound, originalPath, keys, notFoundPath) {
	var value = data;
	...
	if (value instanceof _fragment2.default) {
		...
		if (value._getData && typeof localValue._getData === 'function') {
			value = localValue._getData(path, localValue.getDataIn(), true, onDataNotFoundAll, onDataNotFound);
			localValue.getCounter();
		} else {
			localValue.beforeGet();
			value = localValue.getData(path, localValue.getDataIn(), true, onDataNotFoundAll, onDataNotFound);
			localValue.getCounter();
			localValue.afterGet(value);
		}
	...
	if (path && path instanceof Array) {
		var res = arrayPathParse(path, value, onDataNotFoundAll, onDataNotFound, originalPath || path, keys, nfPath);
		value = res.value;
		if (res.notFoundPath) {
			nfPath = res.notFoundPath;
		}
	} else if (path && (typeof path === 'undefined' ? 'undefined' : (0, _typeof3.default)(path)) === 'object') {
		var _res = objectPathParse(path, value, onDataNotFoundAll, onDataNotFound, originalPath || path, keys, nfPath);
		value = _res.value;
